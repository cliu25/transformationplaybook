        ...
        <div class="panel">
            ...
            <div class="activities-table">
                <div class="activities-header">
                    ...
                    <div class="outcomes-col-header">Why It Matters (Outcome)</div>
                </div>
                ${step.keyActivities.map(activity => `
                    <div class="activity-row">
                        ...
                        <div class="outcome-col">
                            ${typeof activity.outcomes === 'string' ? `<p>${activity.outcomes}</p>` :
                              (activity.outcomes && activity.outcomes.length > 0 ?
                                activity.outcomes.map(outcome => `<p>${outcome}</p>`).join('') :
                                '')}
                        </div>
                    </div>
                `).join('')}
...
function renderModuleDetail(module) {
    ...
    const allSteps = [
        ...
        { id: 'analyze', name: 'Analyze', phaseGroup: 'Engage' },
                                    ...
                                    <div class="accordion-content">
                                        ${sections.map(section => {
                                            ...
                                            ${section.actions && section.actions.length > 0 ? `
                                                <div class="panel">
                                                    <h4>Action → Outcome</h4>
                                                    <div class="action-outcome-list">
                                                        ${section.actions.map(a => {
                                                            if (typeof a === 'string') {
                                                                return `<div class="action-outcome-row"><div class="ao-action">${a}</div></div>`;
                                                            }
                                                            return `<div class="action-outcome-row">
                                                                <div class="ao-action">${a.action || a.title || ''}</div>
                                                                ${a.outcome ? `<div class="ao-arrow">→</div><div class="ao-outcome">${a.outcome}</div>` : ''}
                                                            </div>`;
                                                        }).join('')}
                                                    </div>
                                                </div>
                                                ...
                                                <div class="panel">
                                                    ...
                                                    <ul style="list-style: disc; padding-left: 1.5rem; margin: 0;">
                                                        ${section.commonPitfalls.map(pitfall => `
                                                            <li style="margin-bottom: 0.5rem; color: var(--text-secondary);">${pitfall}</li>
                                                        `).join('')}
                                                    </ul>
                                                </div>
                                            ` : ''}
